...
export function injectPositive(oldValues: number[]): number[] {
    const firstNegativeIndex = oldValues.findIndex((num) => num < 0);
    ...
    if (firstNegativeIndex === -1) {
        ...
        console.log(values);
        const sum = values
            .filter((num, index) => index < firstNegativeIndex)
            .reduce(
                (accumulator, currentValue) => accumulator + currentValue,
                0
            );
        values.splice(firstNegativeIndex + 1, 0, sum);
        console.log(values);
        return values;
    }
}
